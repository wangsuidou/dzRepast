		...
		$(".fontm:has(.pbl) > span").click(function(e){
			...
			$(".nav3 .wrap.fontm > ul").each(function(x,o){
				...
				tabs.click(function(e){
					...
					//if($(e.target).parents(".swipe-nav").length>0) $(e.target).parents(".container.clearfix").first().find(".swipe-slide.slides_wrap > .slide").eq(i).trigger("activate");
					//else
			...
			$(".btn.gray.center.fontm.radiuss.mtl.mbl > .c3[href='#']").bind("click",function(e){
				var currentDiv=$(this).parents(".container.clearfix").eq(0);
	...
	(function($, window, undefined) {
		$(document).ready(function() {
			...
			.each(function(x,o){
				...
				//wrap.parents(".container.clearfix").first().find('.banner-num, .model-num').css('z-index',10);
...
(function(t){
	if($(".container  .banner-num .dot-select").length==0)return;
	var auto_slide=setInterval(function(){	var i=$(".container  .banner-num .dot-select").index();
	var count=$(".container .banner-num").children().length;
	if(i>=count-1)i=0;
	else i+=1;
	$(".container  .banner-num").prev().children(".slide").eq(i).trigger("activate");},t);

})(1000*8);

if(/iPad/.test(navigator.userAgent)){
	$(".container.top-fix").css("position","absolute");
$(window).bind("scroll", function(e){
	$(".container.top-fix").css("top", $(document).scrollTop());
});
}
...
.bind("vchange",function(e){console.log(arguments,this);
    ...
    $(function ($) {
        ...
        $(".banner-num, .model-num").each(function (i, o) {
            ...
            for (var k = 0; k < count; k++)
                $dots.push('<a class="radiusl left mrm dot"></a>');

            $(this).empty()
                .append($dots.join("")).width((count + 1) + "em").children()
